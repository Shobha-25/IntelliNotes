...
function FinalResult({ result }) {
  ...
  if (
    ...
    !data.subTopics ||
    !data.questions ||
    !Array.isArray(data.questions.short) ||
    !Array.isArray(data.questions.long) ||
    !Array.isArray(data.revisionPoints)
  ) {
    return null;
  }

  return (
    <div className="mt-6 space-y-10 bg-white p-3">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            onClick={() => setQuickRevision((value) => !value)}
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          {quickRevision ? (
            <ul className="ml-6 list-disc space-y-2 text-gray-700">
              {data.revisionPoints.length > 0 ? (
                data.revisionPoints.map((point, index) => (
                  <li key={`${point}-${index}`}>{point}</li>
                ))
              ) : (
                <li>No revision points available.</li>
              )}
            </ul>
          ) : (
